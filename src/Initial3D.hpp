...
namespace initial3d {
	...
	public:
		...
		// memory store
		inline friend T * operator<<(T *p, const vec3<T> &v) {
			*p++ = v.m_x;
			*p++ = v.m_y;
			*p++ = v.m_z;
			return p;
		...
		// memory load
		inline friend const T * operator>>(const T *p, vec3<T> &v) {
			v.m_x = *p++;
			v.m_y = *p++;
			v.m_z = *p++;
			return p;
		...
		inline static vec3<T> one(const T &t) {
			return vec3<T>(t, t, t);
		}
	...
	public:
		...
		// memory store
		inline friend T * operator<<(T *p, const vec4<T> &v) {
			*p++ = v.m_x;
			*p++ = v.m_y;
			*p++ = v.m_z;
			*p++ = v.m_w;
		...
		// memory load
		inline friend const T * operator>>(const T *p, vec4<T> &v) {
			v.m_x = *p++;
			v.m_y = *p++;
			v.m_z = *p++;
			v.m_w = *p++;
